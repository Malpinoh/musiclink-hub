...
const ShareButtons = ({ url, title, artist, compact = false }: ShareButtonsProps) => {
  ...
  const handleCopy = async () => {
    ...
    trackEvent("share_clicked", { method: "copy", title });
    setTimeout(() => setCopied(false), 2000);
  };

  const handleWhatsApp = () => {
    trackEvent("share_clicked", { method: "whatsapp", title });
    window.open(`https://wa.me/?text=${encodeURIComponent(`${text}\n${url}`)}`, "_blank");
  };

  const handleTwitter = () => {
    trackEvent("share_clicked", { method: "twitter", title });
    window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`, "_blank");
  };

  const handleFacebook = () => {
    trackEvent("share_clicked", { method: "facebook", title });
  ...
  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        ...
        trackEvent("share_clicked", { method: "native", title });
      } catch {
        handleCopy();
      }
    } else {
